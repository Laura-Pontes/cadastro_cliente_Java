...
•	Read (Carregar/Exibir): A leitura dos clientes ocorre através do método responsável por consultar todos os registros da tabela e preencher a interface. Cada linha obtida no banco é adicionada ao modelo da tabela, permitindo a visualização imediata.
...
O design do projeto foi feito de modo simples e direto:
...
•	Banco de dados SQLite: O banco local foi escolhido pela praticidade, pois não requer instalação de servidor e funciona bem para aplicações pequenas.
